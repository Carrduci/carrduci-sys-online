...
} from '../../ux/varios/tabla-generica/tabla-generica.component';
...
export class VistaFolioDetalleComponent implements OnInit, AfterViewInit {
  ...
  folio_vendedor: WritableSignal<FolioVendedorPublicoRecibir | undefined> =
    signal(undefined);
  folio_vendedor_original: WritableSignal<
    FolioVendedorPublicoRecibir | undefined
  > = signal(undefined);
  datos_tabla_generica: WritableSignal<
    | OPCIONES_TABLA_GENERICA<FolioVendedorPublicoRecibir['folioLineas'][0]>
    | undefined
  > = signal(undefined);
  ...
  calcular_precio_total(iva = false) {
    return this.currency_pipe.transform(
      (this.folio_vendedor()
        ?.folioLineas?.map(
          (linea) => linea.cantidad * (linea.precioUnitarioUsado ?? 0),
        )
        ?.reduce((prev, curr) => prev + curr, 0) ?? 0) * (iva ? 1.16 : 1),
      'MXN',
      'symbol',
      '0.2-4',
      'es-MX',
  ...
  obtener_folio_vendedor() {
    ...
    if (ID_FOLIO) {
      this.folio_service
        .PUBLICO_obtener_folio_vendedor_por_id(ID_FOLIO)
        .subscribe({
          next: (folio) => {
            this.folio_vendedor.update((value) => folio);
            this.folio_vendedor_original.update((value) =>
              structuredClone(folio),
            );
            this.precio_total_folio_vendedor = this.calcular_precio_total();
            this.precio_total_con_iva_folio_vendedor =
              this.calcular_precio_total(true);
  ...
  crear_datos_tabla() {
    this.datos_tabla_generica.update((value) => {
      return {
        columns: [
          {
            column_title: '#',
          ...
          },
          {
            ...
            content: {
              callback: (objeto) => {
                return (
                  (objeto.cantidad ?? 0) * (objeto.precioUnitarioUsado ?? 0)
                );
              },
              pipe: CurrencyPipe,
              pipe_args: ['MXN', 'symbol', '0.2-4', 'es-MX'],
  ...
  cambiar_ordenamiento(paginacion: Pagination) {
    ...
    this.folio_vendedor.update((value) => {
      const HAY_ORDENAMIENTO =
        Object.keys(paginacion?.sorting_fields ?? {}).length > 0;
  ...
  accion_click_fila(
    datos: OPCIONES_FILA_TABLA_GENERICA<FolioVendedorPublicoRecibir>,
  ) {
    // console.log(datos)
  }
  ...
  solicitar_confirmacion_aprobado() {
    this.id_dialogo_confirmacion_aprobado =
      this.notificaciones.crear_notificacion({
        ...
        cuerpo_mensaje:
          '<span class="h6">Una vez que hagas esto, la cotización podrá ser procesada o surtida</span>',
        botones: [
          {
  ...
  aprobar_contizacion() {
    ...
    if (ID) {
      this.folio_service.PUBLICO_aprobar_folio_de_vendedor(ID).subscribe({
        next: (folio) => {
          this.obtener_folio_vendedor();
          this.notificaciones.eliminar_notificacion(
            this.id_dialogo_confirmacion_aprobado,
