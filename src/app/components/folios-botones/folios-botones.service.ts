...
export class FoliosBotonesService extends ServicioGenerico {
    ...
    PUBLICO_obtener_folio_vendedor_por_id(
        ...
        const URL = this.obtener_url(
            [this.ruta_base, 'obtenerCotizacionPublica'],
            {
                id,
            },
        );
        return this.http
            .get<FolioVendedorPublicoRecibir>(URL, this.options)
            .pipe(
                map((respuesta: any) => {
    ...
    PUBLICO_aprobar_folio_de_vendedor(
        ...
        const URL = this.obtener_url(
            [this.ruta_base, 'autorizarCotizacionPublica'],
            { id },
        );

        return this.http
            .put<FolioVendedorPublicoRecibir>(URL, {}, this.options)
            .pipe(
                map((respuesta: any) => {
                    return new FolioVendedorPublicoRecibir(respuesta.datos);
                }),
